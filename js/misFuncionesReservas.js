function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://129.151.108.129:8080/api/Client/all",
        type:"GET",
        datatype:"JSON",
...
function autoInicioBike(){
    ...
    $.ajax({
        url:"http://129.151.108.129:8080/api/Bike/all",
        type:"GET",
        datatype:"JSON",
        success:function(respuesta){
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://129.151.108.129:8080/api/Reservation/save",
...
function listarReservation(){
    $.ajax({
        url:"http://129.151.108.129:8080/api/Reservation/all",
...
function pintarRespuestaReservation(response){
    ...
    myTable+="<tr>";
       ...
       ;
       myTable+='<td><button  onclick="actualizarReservation(' + response[i].idReservation + ')">Actualizar Reservación!</button></td>';
        myTable+='<td><button  onclick="borrarReservation(' + response[i].idReservation + ')">Borrar Reservación!</button></td>';
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://129.151.108.129:8080/api/Reservation/"+idElemento,
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://129.151.108.129:8080/api/Reservation/"+id,
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://129.151.108.129:8080/api/Reservation/update",
